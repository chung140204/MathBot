...
async function main() {
  ...
  ({ classifyTopic, smartClassifyTopic } = await import('../../src/features/knowledge/lib/rag/router'));
  ({ createEmbedding } = await import('../../src/features/knowledge/lib/rag/embed'));
  ({ ragSearch } = await import('../../src/features/knowledge/lib/rag/pipeline'));
  ({ decomposeQuery } = await import('../../src/features/knowledge/lib/rag/decompose'));
  ({ detectFollowUp } = await import('../../src/features/knowledge/lib/rag/query-rewriter'));
  ({ mergeAndRerank } = await import('../../src/features/knowledge/lib/rag/rerank'));
  ...
  // ═══════════════════════════════
  console.log('\n📌 1b. LLM Topic Router (smartClassifyTopic — cần API key)');
  // ═══════════════════════════════

  await testAsync('câu mơ hồ "tim m de PT co 3 nghiem" → FUNCTIONS (LLM)', async () => {
    try {
      const topic = await smartClassifyTopic('tìm m để phương trình có 3 nghiệm');
      console.log(`     → LLM classified: ${topic}`);
      return topic === 'FUNCTIONS' || topic === 'DERIVATIVES';
    } catch (e: any) {
  ...
  await testAsync('câu về tích phân diện tích → INTEGRALS (LLM)', async () => {
    try {
      const topic = await smartClassifyTopic('tính diện tích hình phẳng giới hạn bởi đồ thị');
      console.log(`     → LLM classified: ${topic}`);
      return topic === 'INTEGRALS';
    } catch (e: any) {
      console.log(`     ⚠️  LLM unavailable — skip`);
      return true;
    }
  });
  ...
  await testAsync('search "đạo hàm" → chunks thuộc DERIVATIVES hoặc có relatedTopics DERIVATIVES', async () => {
    const { chunks } = await ragSearch('công thức đạo hàm cơ bản');
    return chunks.every((c: any) =>
      c.topic === 'DERIVATIVES' || c.relatedTopics?.includes('DERIVATIVES')
    );
  });
  ...
  await testAsync('search "cực trị" → chunks có difficulty metadata', async () => {
    try {
      const { chunks } = await ragSearch('tìm cực trị hàm số');
      const withDifficulty = chunks.filter((c: any) => c.difficulty != null);
      console.log(`     → ${withDifficulty.length}/${chunks.length} chunks có difficulty`);
      return withDifficulty.length > 0;
    } catch (e: any) {
      console.log(`     ⚠️  DB error — skip`);
      return true;
    }
  });

  await testAsync('cross-topic: "cực trị hàm bậc 3" → có chunk FUNCTIONS nhờ relatedTopics', async () => {
    try {
      const { chunks } = await ragSearch('cực trị của hàm bậc 3');
      const funcChunks = chunks.filter((c: any) => c.topic === 'FUNCTIONS');
      const derivChunks = chunks.filter((c: any) => c.topic === 'DERIVATIVES');
      console.log(`     → DERIVATIVES: ${derivChunks.length}, FUNCTIONS: ${funcChunks.length}`);
      return chunks.length > 0; // just verify search works
    } catch (e: any) {
      console.log(`     ⚠️  DB error — skip`);
      return true;
    }
  ...
  test('finalScore theo công thức 0.50*vec + 0.27*kw + 0.15*topic + 0.08*relatedBoost', () => {
    ...
    const r = result[0];
    const expected = 0.50 * r.similarity + 0.27 * r.keywordScore + 0.15 * r.topicBoost + 0.08 * r.relatedTopicBoost;
    return Math.abs(r.finalScore - expected) < 0.001;
  });

  test('relatedTopicBoost — chunk relatedTopics chứa detectedTopic → bonus', () => {
    const chunk = {
      id: 'c1', content: 'hàm bậc 3 có cực trị', topic: 'FUNCTIONS', source: 'test',
      similarity: 0.8, relatedTopics: ['DERIVATIVES'],
    };
    const result = mergeAndRerank([[chunk]], [], 'DERIVATIVES', 'cực trị', 5);
    const r = result[0];
    return r.relatedTopicBoost === 1.0 && r.finalScore > 0.50 * 0.8 + 0.15 * 0; // has boost
  });
