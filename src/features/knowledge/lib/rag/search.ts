...
const RAG_SIMILARITY_THRESHOLD = parseFloat(process.env.RAG_SIMILARITY_THRESHOLD || '0.40');
...
export async function searchSimilarChunks(
  ...
  if (topic) {
    ...
    const results = await prisma.$queryRawUnsafe<KnowledgeChunkResult[]>(
      `SELECT id, content, topic, source, difficulty, "subTopic", "relatedTopics",
              1 - (embedding <=> $1::vector) AS similarity
       FROM knowledge_chunks
       WHERE embedding IS NOT NULL
         AND (topic = $4 OR $4 = ANY("relatedTopics"))
      ...
      return await prisma.$queryRawUnsafe<KnowledgeChunkResult[]>(
        `SELECT id, content, topic, source, difficulty, "subTopic", "relatedTopics",
         ...
         WHERE content_tsv @@ plainto_tsquery('simple', $1)
           AND (topic = $3 OR $3 = ANY("relatedTopics"))
